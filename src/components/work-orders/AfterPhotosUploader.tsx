...
import { Camera, AlertCircle, CheckCircle2, Trash2, RefreshCw, Upload, Sparkles } from "lucide-react";
...
export function AfterPhotosUploader({ workOrderId, allPhotos, onUploaded }: Props) {
  ...
  // Filtramos solo fotos After del vehículo (sin servicio ni reporte de inspección asignado)
  const afterVehiclePhotos = allPhotos.filter(
    (p) =>
  ...
  const getSlotPhoto = (label: string) => {
    return afterVehiclePhotos.find((p) => p.caption === label);
  };

  const extraPhotos = afterVehiclePhotos.filter(
  ...
  const hasEnough = afterVehiclePhotos.length > 0;
  ...
  const handleFileChange = async (slotId: PhotoSlotId, label: string, file: File | null) => {
    ...
    try {
      ...
      formData.append("photoType", String(PhotoType.After));
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-extrabold uppercase tracking-wider text-[#041627]">
              Registro Fotográfico Final (Entrega)
            </h2>
          </div>
          <p className="text-xs text-[#44474c]/80 mt-1">
            Suba las 6 fotografías obligatorias que documentan el estado de salida/entrega del vehículo.
          </p>
        </div>
      ...
      <div className="p-4 space-y-4">
        ...
        <div className="space-y-1">
          ...
          <div className="flex justify-between text-[10px] text-[#44474c]/60">
            <span>Progreso de captura final</span>
                    ...
                    <div className="absolute inset-0 bg-[#041627]/70 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 p-2">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          onClick={() => {
                            const input = document.getElementById(`final-input-${slot.id}`) as HTMLInputElement;
